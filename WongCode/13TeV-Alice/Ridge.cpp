...
int n_1 = 80;
double detaf_1 = double((1.8-1.6)/n_1);     //func1 eta 적분범위(Ridge) : 1.6~1.8 -> x2 해야 함.
double etaf0_1 = 1.6;
...
int n_9 = 80;
double detaf_9 = double((4.-2.)/n_9);     //func9 eta 적분범위 : 2~4 -> x2 해야 함.
double etaf0_9 = 2.;
double detajet_9 = double((1.6-0.)/n_1);     //func9 eta 적분범위(Jet) : 0~1.6 -> x2 해야 함.
double etajet0_9 = 0.;
double delta_Deltaphi = 2.56;        //Jet의 delta_Deltaphi
double delta_Deltaphicms = 2.56;

double dphif_9 = double((1.28+1.28)/n_9);   //func9 phi 적분범위 : -1.178~1.178
double phif_9 = -1.28;
double phif0_9 = -1.28;

int n_9_pt = 20;        //pt 포인트당 적분범위 이므로 주의하자. 현재 0.5간격으로 적분중
double dptf_9 = double ((10.-0.)/n_9_pt);  //func1 pt 출력 범위 : 0.1~10.1
double ddptf_9 = dptf_9/n_9;
double ptf_9 = 0.;
int check2_9 = 1;

void func1(){
    double etaf_1, dist_1, sum_1, sum_1j, ptf_11, etajet_1;
    int i_1, j_1, k_1, l_1;
    ...
    for(k_1=1;k_1<=n_1_pt;k_1++){
        for(l_1=1;l_1<=n_1;l_1++){
            ...
            for(j_1=1;j_1<=n_1+1;j_1++){
                ...
                for (i_1=1;i_1<=n_1+1;i_1++){
                    ...
                    sum_1 += ptf_1*frnk(ptf_1)*RidgeDis(Aridge, ptf_1, etaf_1, phif_1, check2_1)*detaf_1*dphif_1*ddptf_1/delta_Deltaeta;
                    ...
                    // std::cout<<ptf_1<<std::setw(20)<<phif_1<<std::setw(20)<<etaf_1<<std::setw(20)<<sum_1<<std::endl;
        ...
        foutalicept<<ptf_1-(dptf_1/2)<<","<<sum_1<<","<<sum_1j<<std::endl;
        // std::cout<<ptf_1<<std::endl;
...
void func9(){
    double etaf_9, dist_9, sum_9, sum_9j, etajet_9;
    int i_9, j_9, k_9, l_9;
    ...
    for(k_9=1;k_9<=n_9_pt+1;k_9++){
        for(l_9=1;l_9<=n_9;l_9++){
            ...
            for(j_9=1;j_9<=n_9+1;j_9++){
                ...
                for (i_9=1;i_9<=n_9+1;i_9++){
                    ...
                    // dist_1 = RidgeDis(Aridge, ptf_1, etaf_1, phif_1, check2_1)*detaf_1*dphif_1/delta_Deltaeta;   //임시저장 용도
                    // sum_1 += RidgeDis(Aridge, ptf_1, etaf_1, phif_1, check2_1)*detaf_1*dphif_1/delta_Deltaeta;
                    sum_9 += ptf_9*frnk(ptf_9)*RidgeDis(Aridge, ptf_9, etaf_9, phif_9, check2_9)*detaf_9*dphif_9*ddptf_9/delta_Deltaetacms;
                    sum_9j += ptf_9*integralNjet(ptf_9,etajet_9,phif_9,constant)*detajet_9*dphif_9*ddptf_9/delta_Deltaphicms;
                    // std::cout<<ptf_1<<std::setw(20)<<phif_1<<std::setw(20)<<etaf_1<<std::setw(20)<<dist_1<<std::endl;
                    etaf_9 += detaf_9;
                    etajet_9 += detajet_9;
                }
        ...
        sum_9 *= 2*2/3;
        sum_9j *= 2;
        foutcmspt<<ptf_9-(dptf_9/2)<<","<<sum_9<<","<<sum_9j<<std::endl;
...
void func2(){
    ...
    for(k_2=1;k_2<=n_2+1;k_2++){
        ...
        sum_2j *= fj;
        // fprintf(fpt,"%f, %f\n", phif, sum);
        std::cout<<k_2<<std::setw(10)<<phif_2<<std::endl;
...
int main()
{
    ...
    fout<<"f_R<N_k>=0.62exp(pT*1.07),"<<endl;
