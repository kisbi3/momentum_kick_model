...
void func2(double pttrig_st, double pttrig_end, double ptf_st, double ptf_end, double etaf_st, double etaf_end, double etacms_st, double etacms_end, double phif_st, double phif_end, int n, int check2, double ptjetcut, double q){
    ...
    int i, j, k;
    double pttrig = (pttrig_st+pttrig_end)/2;
    if(ptjetcut == 0.){
        std::string filename, pttri_st, pttri_end, filename2;
        filename = "phiCorrelation_pt";
        // pttri_st = pttrig_st<1. ? std::to_string(0) : std::to_string(int(pttrig_st));
        pttri_st = std::to_string(int(pttrig_st));
        pttri_end = std::to_string(int(pttrig_end));
        filename.append(pttri_st);
        filename += "-";
        filename.append(pttri_end);
        filename2 = filename;
        ...
        // if(pttrig<1.){
        //     std::cout<<pttrig_st<<std::setw(20)<<pttrig_end<<std::setw(20)<<ptf_st<<std::setw(20)<<ptf_end<<std::setw(20)<<etacms_st<<std::setw(20)<<etacms_end<<std::setw(20)<<phif_st<<std::setw(20)<<phif_end<<std::endl;
        // }
        ...
        for(k=1;k<=n+1;k++){
            ...
            for(i=1;i<=n+1;i++){\
                ...
                for(j=1;j<=n+1;j+=1){
                    ...
                    sum_alice += frnk(pttrig)*ptf*RidgeDis(Aridge, ptf, etaf, phif, check2, q)*dptf*detaf;
                    sum_cms += frnk(pttrig)*ptf*RidgeDis(Aridge, ptf, etacms, phif, check2, q)*dptf*detacms;
                    ptf += dptf;
                }
            ...
            // if(pttrig<1.){
            //     std::cout<<phif<<std::setw(20)<<sum_cms<<std::endl;
            // }
...
//    thread t9(func3, q_1, 0., 50.);
    ...
    thread t2(func2, 0.3, 1, 1., 2., 1.6, 1.8, 2., 4., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pttrig = 2~4
    thread t3(func2, 1, 2, 1., 2., 1.6, 1.8, 2., 4., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pttrig = 4~6
    thread t4(func2, 2, 4, 1., 2., 1.6, 1.8, 2., 4., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pttrig = 6~8
    thread t5(func2, 4, 6, 1., 2., 1.6, 1.8, 2., 4., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pttrig = 8~10
    thread t6(func2, 6, 12, 1., 2., 1.6, 1.8, 2., 4., -1.18, 1.18, 300, 1, 0., q_1);   //1D phi correlation, pttrig = 10~12
    //pttrig_st, pttrig_end, ptf_st, ptf_end, etaf_st, etaf_end, etacms_st, etacms_end, phif_st, phif_end, n, check2, ptjetcut, q
...
//    thread t6(func2, 1., 2., 1.6, 1.8, 1., 2., -1.28, 1.28, 300, 1, 10., q_1);   //1D phi correlation, pt = 1~2 (jet event cut)
//    thread t7(func2, 1., 2., 1.6, 1.8, 1., 2., -1.28, 1.28, 300, 1, 20., q_1);   //1D phi correlation, pt = 1~2 (jet event cut)
